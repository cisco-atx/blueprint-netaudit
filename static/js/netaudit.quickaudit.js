...
$(function () {
     ...
     * and connector credentials, then calling the `runAudit` function.
     */
    $(document).on("click", "#runAuditBtn", function () {
        ...
        RunModal.open(({ config }) => {
            runAudit(devices, checks, config);
        });
    ...
    const checkList = {};
    $.getJSON("/netaudit/api/checks", function (data) {
    ...
    $(document).on("click", "#viewReportBtn", function () {
        $.getJSON("/netaudit/quickaudit/report", function (results) {
            ...
            for (const device in results) {
                ...
                const loginStatusBadge = deviceResult.login ?
                    `<span class="badge status-pass"><span class="material-icons">check_circle</span> SUCCESS</span>` :
                    `<span class="badge status-fail"><span class="material-icons">cancel</span> FAILED</span>`;
    ...
    $(document).on("click", "#exportExcelBtn", function () {
        ...
        $.getJSON("/netaudit/quickaudit/report", function (results) {
            ...
            // send to server
            fetch("/netaudit/quickaudit/export", {
                method: "POST",
                headers: {
     ...
     * @param {Object} connector - Connector credentials for Jumphost and network.
     */
    function runAudit(devices, checks, connector) {
        ...
        runAuditBtn.prop("disabled", true)
            .html('<span class="material-icons spin">autorenew</span> Running...');

        $.ajax({
            url: "/netaudit/quickaudit/run",
            method: "POST",
            contentType: "application/json",
            data: JSON.stringify({ devices, checks, connector }),
            success: (resp) => {
                if (resp.success) {
            ...
            complete: () => {
                runAuditBtn.prop("disabled", false)
                    .html('<span class="material-icons button">play_arrow</span> Run Audit');
            }
        });
    }
