...
$(document).ready(function () {
    ...
    $(document).on('click', '.edit-btn', function () {
        ...
        $.getJSON('/netaudit/api/devices', function (data) {
            const device = data[key];
            if (!device) return;
            $('#deviceHostnames').val(key).prop('disabled', true);
            setSelectedViews(device.view || []);
            $('#deviceConnector').val(device.connector || '');
    ...
    $('#modalForm').on('submit', function (e) {
        ...
        const connectorVal = $('#deviceConnector').val();

        const data = {
            view: $('#viewDropdownMenu input:checked')
                .map(function () { return this.value; })
                .get(),
            connector: connectorVal ? connectorVal : null
        };
